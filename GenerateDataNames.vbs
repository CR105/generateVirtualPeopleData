...
For Iterator = 1 To 10 Step 1
	Set data = getVirtualPersonData()
	objDriverSheetW.cells(Iterator+1, 1).Value = data("Nombre")
	objDriverSheetW.cells(Iterator+1, 2).Value = data("ApellidoPaterno")
	objDriverSheetW.cells(Iterator+1, 3).Value = data("ApellidoMaterno")
	objDriverSheetW.cells(Iterator+1, 4).Value = data("FechaNacimiento")
	objDriverSheetW.cells(Iterator+1, 5).Value = data("CiudadNacimiento")
	objDriverSheetW.cells(Iterator+1, 6).Value = data("Genero")
	objDriverSheetW.cells(Iterator+1, 7).Value = data("CURP")
	objDriverSheetW.cells(Iterator+1, 8).Value = data("RFC")
Next
...
Public Function getVirtualPersonData()
	...
	objData.add "FechaNacimiento", Cstr(RandomNumber(1950,1999)) & "/" & Left("0" + Cstr(RandomNumber(1,12)), 2) & "/" & Left("0" + Cstr(RandomNumber(1,28)), 2)
	objData.add "Genero", objDicGen.Item(Cstr(intGenero))
	objData.add "CiudadNacimiento", objDriverSheetR.cells(intCiudad+1, 4)
	...
	msgbox objData("CiudadNacimiento")
